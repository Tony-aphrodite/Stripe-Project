 ...
 *
 * Access: /configurador/php/truora-iframe-diag.php?key=voltika_truora_2026
 */
...
function truoraDiagPingUrl(string $url, string $apiKey, string $flowId, string $voltikaBase): array {
    $body = http_build_query([
        ...
        'redirect_url' => rtrim($voltikaBase, '/') . '/configurador/',
    ]);
    $t0 = microtime(true);
    $ch = curl_init($url);
...
<div class="kv">
    ...
    <div><?= htmlspecialchars(rtrim($voltikaBase, '/') . '/configurador/php/truora-webhook.php') ?></div>

    <div>Endpoint token</div>
    <div><?= htmlspecialchars(rtrim($voltikaBase, '/') . '/configurador/php/truora-token.php') ?></div>
</div>
</div>
