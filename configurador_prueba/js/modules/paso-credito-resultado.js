...
var PasoCreditoResultado = {
    ...
    bindEvents: function() {
        ...
        jQuery(document).on('click', '#vk-nv-retry', function() {
            ...
            // every state field that Paso4B init() will read. This is
            // the third belt of "belt-and-suspenders" — even if Paso4B
            // self-determines from _resultadoFinal, we also pin the
            // scalar fallbacks here so any future code path that loses
            // _resultadoFinal still finds modoCondicional + bounds.
            if (isCondicional) {
