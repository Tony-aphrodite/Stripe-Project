...
var PasoCreditoIdentidad = {
    ...
    render: function() {
        ...
        // (RENAPO government DB). Without a valid CURP, Truora can only do
        // weak name+DOB matching and returns not_found for most users.
        // Gender is derived from CURP position 11 so no separate selector.
    ...
    _verificar: function() {
        ...
        // CURP — required for Mexican government identity lookup. We also
        // derive gender from position 11 (H=hombre, M=mujer) so we don't need
        // a separate gender selector in the UI. Validate format client-side
        // to avoid round-tripping to the backend only to get a 400.
        ...
        state.curp = curpRaw;
        var genderFromCurp = curpRaw.charAt(10); // 'H' or 'M' in Mexican CURP
        var truoraGender = genderFromCurp === 'M' ? 'F' : 'M'; // Truora uses M/F
